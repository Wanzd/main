...
public class DbService implements IService<VO>, IBatchService<VO>
{
    ...
    @RequestMapping(value = "/ra", method = {RequestMethod.GET,
        RequestMethod.POST}, produces = "text/html;charset=utf-8")
    public String ra(@RequestParam Map<String, String> in)
    {
        VO fo = AI.build(in, new Map2VO());
        IDbDao dao = getDao(fo);
        List<VO> ra = dao.ra();
        String rsStr = JSON.toJSONString(ra);
        return rsStr;
    }
    ...
    @RequestMapping(value = "/rs", method = {RequestMethod.GET,
        RequestMethod.POST}, produces = "text/html;charset=utf-8")
    public String rs(@RequestParam Map<String, String> in)
    {
        VO fo = AI.build(in, new Map2VO());
        IDbDao dao = getDao(fo);
        List<VO> ra = dao.rs(fo);
        String rsStr = JSON.toJSONString(ra);
        return rsStr;
    }
